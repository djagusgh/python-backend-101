...
# 19.06.22
# 책 코드 중 app.config['JWT_SECRET_KEY'] : 'JWT_SECRET_KEY' keyError 발생
# -> app.config['SECRET_KEY']를 사용해야 하고, 초기값을 지정해줘야 한다.(이 코드에서는 encryption_secret으로 초기화)

encryption_secret = 'secrete'

## Default JSON encoder는 set을 JSON으로 변환할 수 없다.
## 그러므로 custom encoder를 작성해 set를 list로 변환하여
## JSON으로 변환가능하게 해주어야 한다.
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if access_token is not None:
            try:
                payload = jwt.decode(access_token, current_app.config['SECRET_KEY'], 'HS256')
...
def create_app(test_config = None):
    ...
    app.config['SECRET_KEY'] = encryption_secret
    ...
    def login():
        ...
        else: # 사용자가 존재하지 않거나 사용자의 비밀번호가 틀린 경우
            return '', 401
